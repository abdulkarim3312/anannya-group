...
class ProductController extends Controller{
    ...
    public function productAddPost(Request $request){
        ...
        $rules = [
            ...
            'image' => 'required',
            'quotation' => 'required|file|mimes:pdf|max:2048', // 2MB max size
            'catalogue' => 'required|file|mimes:pdf|max:2048', // 2MB max size
    ...
    public function productImageUpload(Request $request){
        try {
            ...
            $filename = Uuid::uuid1()->toString().'.'.$file->getClientOriginalExtension();
            $destinationPath = 'uploads/product_image/thumbs';
            $file->move($destinationPath, $filename);
