    ...
    <script>
        ...
        $(function() {
            ...
            $('#table').DataTable({
                processing: true,
                serverSide: true,
                ajax: '{{ route('new_setup_amount_receipt.datatable') }}',
                columns: [
                    {data: 'name', name: 'name'},
            ...
            $('body').on('click', '.btn-receive', function() {
                ...
                $.ajax({
                    method: "GET",
                    url: "{{ route('new_setup_amount_receipt.get_orders') }}",
                    data: { employeeId: employeeId }
                }).done(function( response ) {
                    $.each(response, function( index, item ) {
                        $('#modal-order').append('<option value="'+item.id+'">'+item.service.complain_job_id+'</option>');
                    });
            ...
            $('#modal-order').change(function () {
                ...
                if (orderId != '') {
                    $.ajax({
                        method: "GET",
                        url: "{{ route('get_new_setup_order_details') }}",
            ...
            $('#modal-btn-receipt').click(function() {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ route('new_setup_receipt.make_payment') }}",
                    data: formData,
                    processData: false,
                    contentType: false,
